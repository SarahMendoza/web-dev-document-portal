// import "./AdminCreateFormPage.css";
...
const AdminCreateFormPage = () => {
  ...
  const handleSaveForm = async () => {
    ...
    try {
      const res = await axios.post(
        `http://localhost:8080/form/new/${username}`,
        body
      );
  ...
  const handleSubmitForm = async () => {
    ...
    try {
      ...
      // publish
      await axios.post("http://localhost:8080/form/publish", created, {
      ...
      <div className="inline-container">
        {isLoading && !selectedTemplate ? (
          ...
          <div className="error-message">
            {error}
            <button onClick={fetchFormTemplates}>Retry</button>
          </div>
        ) : (
          <>
            <DropdownButton
